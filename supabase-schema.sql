...
CREATE TABLE usage_logs (
    ...
    request_type TEXT NOT NULL CHECK (request_type IN ('rephrase', 'generate', 'grammar', 'formal', 'casual', 'analyze', 'reply', 'extract', 'custom', 'dictation', 'dictation_streaming', 'dictation_deepgram')),
...
-- =============================================
-- PUSH NOTIFICATION HISTORY (admin audit log)
-- =============================================
...
CREATE INDEX IF NOT EXISTS idx_push_notification_history_created ON push_notification_history (created_at DESC);

ALTER TABLE push_notification_history ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Service role full access push_notification_history" ON push_notification_history
    FOR ALL USING (auth.role() = 'service_role');
